      ...
      emprendimiento: [
        {
          ...
          descripcion:
            "CERRAJERO. Soluciones de cerrajería - Blindajes - Seguridad Electrónica",
          rubro: "Cerrajeria",
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  inputBusqueda.addEventListener("input", () =>
    manejarEntradaBusqueda(inputBusqueda, lista)
  );
});

function manejarEntradaBusqueda(inputBusqueda, lista) {
  console.log(inputBusqueda);
  if (inputBusqueda.value === "") {
    lista.innerHTML = "";
  } else {
    let busqueda = inputBusqueda.value.toLowerCase();
    let resultados = filtrarResultados(busqueda);
    let resultadosOrdenados = ordenarResultados(resultados);
    insertarResultadoBusqueda(resultadosOrdenados, lista);
  }
}

function filtrarResultados(busqueda) {
  return datos.colaboradores.filter((colaborador) =>
    colaborador.emprendimiento.some((emprendimiento) =>
      Object.values(emprendimiento).some(
        (valor) =>
...
function ordenarResultados(resultados) {
  return resultados.sort((a, b) => {
    let aDestacado = a.emprendimiento.some(
      (emprendimiento) => emprendimiento.destacado
    );
    let bDestacado = b.emprendimiento.some(
      (emprendimiento) => emprendimiento.destacado
    );
...
function insertarResultadoBusqueda(resultados, lista) {
  lista.innerHTML = "";
  if (resultados.length > 0) {
    resultados.forEach((colaborador) => {
      colaborador.emprendimiento.forEach((emprendimiento) => {
        lista.append(crearBusquedaEncontrada(emprendimiento));
      });
    });
  } else {
    lista.append(crearBusquedaNoEncontrada());
  }
}

function crearBusquedaEncontrada(emprendimiento) {
  let listItem = document.createElement("li");
  listItem.textContent = `${emprendimiento.nombre}, ${emprendimiento.direccion}, ${emprendimiento.rubro}`;
  ...
  listItem.addEventListener("click", () => {
    window.location.href = `emprendimiento.html?id=${emprendimiento.id_emprendimiento}`;
  });

  return listItem;
}

function crearBusquedaNoEncontrada() {
  let listItem = document.createElement("li");
